...
> 🎛️ DSP 事件驱动引擎 + 音量门限/强度映射 + 多项修复

- **DSP 事件驱动触觉引擎**：独立 DSP 线程多频段实时分析（KICK/SNARE/SUB/TICK/BODY/VOCAL），节拍检测才触发震动——替代原连续拉取模式，震感更有音乐性
- **新增系统音量门限**：媒体音量低于门槛时完全静音（静音保护），音量还参与强度缩放（类似增益档位）
- **新增强度上限** `intensity_cap`：限制最大输出增益，防止烧马达
- **震动阈值语义修正**：`haptic_threshold` 现在作为弱拍过滤（rms 域）——信号强度低于阈值线不触发，而非之前错误的输出强度过滤
- **震动模式修正**：鼓点→只留 KICK；低音→KICK+SUB；智能→全部节拍（修复中文映射）
- **定时开关（安静时段）生效**：DSP worker 触发路径同步检查，晚间自动静音
- **per-app「启用此应用触觉」开关生效**：关闭后注入进程不再触发震动
- **Phira 音游支持**：谱面驱动触觉路径
- **Root 直驱回退**：马达节点直驱（SELinux 阻止时自动降级）
- **C++ native 全量移植**：5 频段分频/音高估计/缓冲，与 Kotlin 双轨融合
- **修复播放闪退**：双线程 native 竞争（Scudo double-free）——DSP 线程独占 native 引擎
- **修复 UI 频谱/触觉动态静止**：ByteBuffer 字节序错误导致 native 遥测全 0；HapticComposer 震动遥测缺失导致波形无幅度
- **修复定时开关阻断遥测**：安静时段主循环空转导致频谱数据不更新
- **诊断日志持续落盘**：自动写入日志文件，每天自动清空（无需手动导出）
